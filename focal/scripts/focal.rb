...
class Focal
  def self.configure(config, settings)
    ...
    # Configure The Box
    config.vm.define settings['name'] ||= 'focal64'
    config.vm.box = settings['box'] ||= 'ubuntu/focal64'
    config.vm.hostname = settings['hostname'] ||= 'focal64'
    ...
    if settings.has_key?('backup') && settings['backup'] && (Vagrant::VERSION >= '2.1.0' || Vagrant.has_plugin?('vagrant-triggers'))
      ...
      settings['databases'].each do |database|
        ...
        if (enabled_databases.include? 'mysql') || (enabled_databases.include? 'mariadb')
          Focal.backup_mysql(database, "#{dir_prefix}/mysql_backup", config)
        end
      end
    end
  end
